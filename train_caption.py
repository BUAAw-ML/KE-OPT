...
from model.caption import OPTForCaption
...
### overwrite video_cfg with customize settings
    ...
    # train_dataset = TxtVideoDataset(opts.train_ids_path, txt_mapper, video_mapper_train)

    # train_dataloader = build_dataloader(train_dataset, txtvideo_collate, True, opts)
     
    # test_dataset = TxtVideoDataset(opts.test_ids_path,  txt_mapper, video_mapper_test)
    # test_loader = build_dataloader(test_dataset, txtvideo_collate, False, opts)

    train_dataset = TxtVideoAudioDataset(opts.train_ids_path, txt_mapper, video_mapper_train,audio_mapper)

    train_dataloader = build_dataloader(train_dataset, txtvideoaudio_collate, True, opts)
     
    test_dataset = TxtVideoAudioDataset(opts.test_ids_path,  txt_mapper, video_mapper_test, audio_mapper)
    ...
    opts.model_cfg['beam_size'] = opts.beam_size
    ...
    eval_log, results_2m, results_3m = evaluate(model, test_loader,  opts)
    ...
    if hvd.rank() == 0:
        ...
        json.dump(results_2m,open(join(result_folder, 'onlytest_2m.json'), 'w'))
        json.dump(results_3m,open(join(result_folder, 'onlytest_3m.json'), 'w'))      
    if opts.zero_shot:
        return 
    ...
    while True:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if sample_num_3m > 0:
                loss_3m = loss_dict['caption_loss_3m']
            else:
                loss_3m = 0
            
            if sample_num_2m > 0:
                loss_2m = loss_dict['caption_loss_2m']
            else:
                loss_2m = 0
                    ...
                    if hvd.rank() == 0:
                        for eval_name, metric in eval_log.items():
                            ...
                            if ('best_step' not in metric_logger_dict[eval_name]) or \
                                    (metric['CIDEr'] >= metric_logger_dict[eval_name]['best_value']):
                                metric_logger_dict[eval_name]['best_step'] = global_step
                                metric_logger_dict[eval_name]['best_value'] = metric['CIDEr']
                        ...
                        json.dump(results_3m,open(join(result_folder, 'step_{}_3m.json'.format(global_step)), 'w'))
...
def evaluate(model, eval_loader , opts):
    ...
    for batch in eval_loader:
        ...
        if sample_num_3m > 0:
            sents = evaluation_dict['generated_sequence_3m']
            sents_woaudio = evaluation_dict['generated_sequence_3m_woaudio']
            sents = decode_sequence(i2v, sents.data, eos_token)
            sents_woaudio = decode_sequence(i2v, sents_woaudio.data, eos_token)

            for  i in range(sample_num_3m):
                result_3m = {'video_id':ids_3m[i], 'caption': sents[i]}
                results_3m.append(result_3m)
                result_3m_woaudio = {'video_id':ids_3m[i], 'caption': sents_woaudio[i]}
                results_2m.append(result_3m_woaudio)
    ...
    all_results_3m = [i for results_3m in all_gather_list(results_3m)  for i in results_3m]
    ...
    #ipdb.set_trace()
    metric_2m = compute_metric(all_results_2m, opts)  
    metric_3m = compute_metric(all_results_3m, opts)  
    eval_log = {'metric_2m': metric_2m, 
                'metric_3m': metric_3m, }
    tot_time = time()-st

    LOGGER.info(f"evaluation finished in {int(tot_time)} seconds")
    return eval_log, all_results_2m, all_results_3m
...
def compute_metric(results, opts):

    coco = COCO(opts.annfile)
