...
vit_weight = np.load("./output/pretrianed_weights/ViT-B_32.npz")

opt_weight['opt.audio_embeddings.cls_token']  = trans(vit_weight['cls'])
...
opt_weight['opt.audio_embeddings.first_conv.weight'] =  first_conv_weight  ### need to permute?
opt_weight['opt.audio_embeddings.first_conv.bias'] = trans(vit_weight['embedding/bias'])
...
pos_weight_oth = pos_weight_oth.reshape(7,7,-1).permute(2,0,1).unsqueeze(0)
pos_weight_oth = F.interpolate(pos_weight_oth, size = (4,32),mode='bilinear').squeeze().permute(1,2,0).reshape(-1,768)
pos_weight_oth = torch.cat((pos_weight_cls,pos_weight_oth),dim=0)
opt_weight['opt.audio_embeddings.position_embeddings.weight'] = pos_weight_oth

print(pos_weight_oth.shape)



for  i in range(12):
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.attention.linears.0.weight'] = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MultiHeadDotProductAttention_1/query/kernel']).reshape(768,-1).permute(1,0)
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.attention.linears.0.bias'] = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MultiHeadDotProductAttention_1/query/bias']).reshape(768)
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.attention.linears.1.weight'] = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MultiHeadDotProductAttention_1/key/kernel']).reshape(768,-1).permute(1,0)
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.attention.linears.1.bias'] = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MultiHeadDotProductAttention_1/key/bias']).reshape(768)
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.attention.linears.2.weight'] = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MultiHeadDotProductAttention_1/value/kernel']).reshape(768,-1).permute(1,0)
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.attention.linears.2.bias']  = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MultiHeadDotProductAttention_1/value/bias']).reshape(768)
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.attention.linears.3.weight']  = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MultiHeadDotProductAttention_1/out/kernel']).reshape(-1,768).permute(1,0)
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.attention.linears.3.bias'] = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MultiHeadDotProductAttention_1/out/bias'])
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.ff_layer.linear1.weight']  = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MlpBlock_3/Dense_0/kernel']).permute(1,0)
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.ff_layer.linear1.bias']  = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MlpBlock_3/Dense_0/bias'])
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.ff_layer.linear2.weight']  = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MlpBlock_3/Dense_1/kernel']).permute(1,0)
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.ff_layer.linear2.bias']  = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/MlpBlock_3/Dense_1/bias'])
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.layernorm1.weight']  = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/LayerNorm_0/scale'])
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.layernorm1.bias']  = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/LayerNorm_0/bias'])
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.layernorm2.weight']  = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/LayerNorm_2/scale'])
    opt_weight['opt.audio_encoder.layer.'+str(i)+'.layernorm2.bias'] = trans(vit_weight['Transformer/encoderblock_'+str(i)+'/LayerNorm_2/bias'])
opt_weight['opt.audio_encoder.last_layernorm.weight'] = trans(vit_weight['Transformer/encoder_norm/scale'])
opt_weight['opt.audio_encoder.last_layernorm.bias'] = trans(vit_weight['Transformer/encoder_norm/bias'])
...
torch.save(opt_weight,'./output/pretrianed_weights/v3.0_bertbaseuncased_timesformer16_timezero_ast32.pt')
