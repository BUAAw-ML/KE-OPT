...
from data import (PrefetchLoader,VideoMapper)
#from data.data import TxtVideoDataset, txtvideo_collate
from data.image_classification import ImageClassificationDataset ,  imageclassification_collate

from optim import get_lr_sched
from optim.misc import build_optimizer

from model.imageclassification import OPTForImageClassification
...
### overwrite video_cfg with customize settings
    ...
    data_type = opts.data_type if opts.data_type else 'image_downstream'
    ...
    label_json_path = opts.label_json
    #audio_mapper = AudioMapper(opts.audio_path, opts.audio_cfg, data_type)

    # ipdb.set_trace()
    # img=video_mapper_test['video9770']

    train_dataset = ImageClassificationDataset(opts.train_ids_path, video_mapper_train, label_json_path) ### when training and onev-to-manyt, a random t is choosed.
    train_dataloader = build_dataloader(train_dataset, imageclassification_collate, True, opts) 

    
    test_dataset = ImageClassificationDataset(opts.test_ids_path, video_mapper_test, label_json_path)
    test_loader = build_dataloader(test_dataset, imageclassification_collate, False, opts)
    ...
    model = OPTForImageClassification.from_pretrained(
                    ...
                    if hvd.rank() == 0:
                        ...
                        for eval_name, metric in eval_log.items():
                            ...
                            if ('best_step' not in metric_logger_dict[eval_name]) or \
                                    (metric['top1_acc'] >= metric_logger_dict[eval_name]['best_value']):
                                metric_logger_dict[eval_name]['best_step'] = global_step
                                metric_logger_dict[eval_name]['best_value'] = metric['top1_acc']
...
def evaluate(model, val_loader):
    model.eval()
    LOGGER.info("start running image classification validation...")
    ...
    for i, batch in enumerate(val_loader):
        ...
        top1_acc_num += (logits.max(dim=-1)[1] == labels).sum().item()
        
        top5_acc_num += len((logits.topk(5,dim=-1)[1] == labels.unsqueeze(1).expand(-1,5)).nonzero())
        total_num += len(labels)
        
        pbar.update(1)
    pbar.close()
    all_top1_acc_num =  sum(all_gather_list(top1_acc_num))
    all_top5_acc_num =  sum(all_gather_list(top5_acc_num))
    all_total_num =  sum(all_gather_list(total_num))

    if hvd.rank()==0:
        eval_log = {'top1_acc': round((all_top1_acc_num / all_total_num) * 100, 1),
                    'top5_acc': round((all_top5_acc_num / all_total_num) * 100, 1)}

        eval_log = {'image_classification': eval_log}
    else:
        eval_log = None
    model.train()
    return eval_log
